...
floatType TNM_SPATH::PathCostS()
{
	floatType c = 0.0;
	for (vector<TNM_SLINK*>::iterator pv = path.begin(); pv!=path.end(); pv++)	c += (*pv)->cost;
	return c;
}
...
void TNM_SNET::SetLinkTollType(TNM_TOLLTYPE tl)
{
	for(int i = 0;i<numOfLink;i++)
	{
		if(!linkVector[i]->dummy)
			linkVector[i]->SetTollType(tl);
	}
}
...
int TNM_SNET::BuildTAPAS(bool loadod, const TNM_LINKTYPE ltype)
{
	...
	nodefilename = networkName + "_nod.tntp";
	tolfilename = networkName + "_tol.tntp";
	int nn, nl, nz,nft;
	bool readNode = true;
			...
			if(words[0].find_first_of("~") == -1 && words.size() >= 10)
			{
				...
				if(tail !=head)
				{
					...
					if(words[9].size()>=4) //type string larger than
					{
						...
						str>>tltype; //temporary link type
						lval.type = tltype;
					}
					//check if capacity is abnormal, often centroid connector's capacity are set to 0.0. 
					if(cap <=1e-6)
					{
					...
					TNM_FromString<floatType>(link->cost, words[8], std::dec); //read in toll into cost, note that this is only temporary.
					TNM_FromString<floatType>(link->toll, words[8], std::dec);//now read toll into the permanent cost. 
					//set toll type.
					link->SetTollType(TT_NOTOLL); //initialize all to be no toll.  In most cases, toll type will be specified globally when solving TAP.
...
int TNM_SNET::BuildSZFormatPS()
{
	...
	nodecsv.open(networkName+"\\road_node.csv");
	linkcsv.open(networkName+"\\road_link.csv");
	string line;
...
//Shortest path calculation provided address of a given origin
...
bool SCAN_DEQUE::InsertANode(TNM_SNODE *node)
{
	...
	{
	if(node->scanStatus == 0)
	{
		nodeList.push_back(node); 
	    node->scanStatus = 1;
		return true;
	}
    else if (node->scanStatus == -1)
	{
