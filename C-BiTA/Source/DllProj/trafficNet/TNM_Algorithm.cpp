...
void TAP_SBA::MainLoop()
{
	...
	{
		pOrg = network->originVector[i];
		//cout<<"origin"<<pOrg->origin->id<<endl;
		floatType oldtreenum = spptreenum;
	...
	int innerI=0;
	int innerMaxI=300;//3000 works better.
	int innerFC=0;
	double migap=50;
	...
	while(innerI<innerMaxI)
	{
		...
		migap=0;
		if (convIndicator<1e-6)//1e-8
		{
			innerMaxI=600;//3000 works better
		}
		innerI += 1;
		innerFC = 0;
		for (int i=0;i<network->numOfOrigin;i++)
		{
			...
			for (int j=0;j<pOrg->numOfDest;j++)
			{
				...
				//}
				//
				// innerFC = adaptive_inner_2(deltaod, innerFC, dest);
				if (deletestrategy == 1)
				{
					if (innerI%5==0)
					{
