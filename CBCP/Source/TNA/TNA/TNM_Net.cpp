...
floatType TNM_SLINK::GetDer2Cost(bool ftoll)
{
	//double t, v = volume;
	//if(m_classid >0)
	//{
...
bool TNM_BPRLK::Initialize(const LINK_VALCONTAINER cont)
{
	...
	{
		 alpha = cont.par[0]; //if (cont.par[0]>=0.0 && cont.par[0]<999.0)
		 beta  = cont.par[1]; //if (cont.par[1]>=0.0 && cont.par[1]<99.0)  
	}
...
///////////////////bcp link type definition
//
//bool TNM_BCPLK::Initialize(const LINK_VALCONTAINER cont)
//{
//	if (cont.par.size()==2) //in general this is not in use.
//	{
...
void TNM_SNET::UpdateLinkCost(bool fToll)
{
	...
	for(int i = 0;i<numOfLink;i++)
	{
		...
		//cout<<link->tail->id<<"-->"<<link->head->id<<" : "<<link->volume<<"  "<<link->cost<<endl;
	}
