...
public:
	...
	vector<PATHELEM*>   pathElemVector;
...
public:
	...
	floatType			  PathCostS(); //compute static path cost by simply adding link-cost and return it. path cost will not be updated.
	int                   GetLinkNum() {return path.size();}
private:
...
public:
	...
	vector<floatType>  pathSetbound;//Used to solve the Bicriteria Shortest Path Problem
	vector<TNM_SPATH*> rdmSet;
};
