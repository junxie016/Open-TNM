...
public:
	...
	virtual void		      Print(bool node = false);
};
...
public:
	...
	TNM_SPATH*           SPath(TNM_SNODE* origin, TNM_SNODE* dest); // calculate the shortest path using the one-to-one Dijkstra
	TNM_SPATH*           GetSPath_R_(TNM_SNODE* origin, TNM_SNODE* dest); //retrieve a shortest path from a dest-based tree.

	void                UpdateSPR(TNM_SNODE* rootDest); //calculate the shortest path for a given destination
	void                UpdateSPR(TNM_SNODE* origin, TNM_SNODE *dest); //calculate the shortest path for a given destination
	...
	void                 InitialSubNet3(int sptreeLen = 0, bool createpath = false); //perform the ALL-OR-NOTHING assignment using an one-to-many shortest path algorithm
	void                 InitialSubNet4(int sptreeLen = 0, bool createpath = false); // perform the ALL - OR - NOTHING assignment using an one - to - one shortest path algorithm
private:
	int				    nodeBufferSize;
	int			        linkBufferSize;
	int				    pathBufferSize;
...
public:
	...
	void         SPTreeD(TNM_SNODE *origin, TNM_SNODE *dest); //given destination, perform...

	void         SPTreeD(TNM_SNODE*); //given destination, perform...
	virtual bool InsertANode(TNM_SNODE *) {return false;}
	virtual bool InsertANode2(TNM_SNODE*) { return false; }
	virtual            TNM_SNODE* GetNextNodeMin() { return false; };
protected:
...
public:
	...
	virtual bool       InsertANode2(TNM_SNODE*);

protected:
	
	virtual            TNM_SNODE *GetNextNode();
	void	           QuickSortNode(int low, int high);
	virtual            TNM_SNODE* GetNextNodeMin();
