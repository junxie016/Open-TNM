...
typedef map<int, KSPPATHELEM*, std::less<int> > KSPMAP;
typedef multimap<floatType, KSPQUEUE, std::less<floatType> > KSPMTMAP;
...
public:
	...
	int                attachedDest;
	bool			   is_centroid; //
	bool			   SkipCentroid;//if need, skip centroid on the ksp algorithm.
	KSPMAP*            kspPathElem; //for K-Shortest path algorithm.
...
public:
	...
	floatType			beckFlow;  /*preload flow*/
	...
	bool				NoEntry;     //路段禁止通行
	vector<floatType>	mc_volume;  //各用户的流量
	vector<floatType>	mc_beckFlow; //各用户的预加载流量
	floatType			period;//Ackelik费用函数的参数，分配的时段长度
	floatType			walktime;//路段走行时间，用于公交分配
	floatType			ivtt;//用于公交分配
protected:
...
public:
	...
	floatType           GetCost(floatType vot, bool ftoll = false); 
...
class TNM_EXT_CLASS TNM_ALKLK :	public TNM_SLINK
{
public:
//constructors and destructor
	TNM_ALKLK(void) { period=1; type = ALKLK;}
	virtual ~TNM_ALKLK(void) {;}

	virtual bool		Initialize(const LINK_VALCONTAINER cont); //the first of cont.par is given to alpha, the other is given to beta.
	//void                SetAlpha
	floatType  GetIdelay() {return Idelay;}
	floatType  GetCoe()  {return coe;}
	//floatType  GetDownCap()  {return downCap;}
protected:
	floatType			Idelay; //inherent delay
	floatType			coe; //coefficient of delay
	//floatType			downCap;// downstream capacity
	virtual floatType   GetCost_();
	virtual floatType   GetDerCost_();
	virtual floatType   GetIntCost_();
	virtual floatType   GetDer2Cost_();
...
public:
	...
	vector<destInfo*>	 destInfoList;  //在深圳交通中心网络格式中使用
...
public:
	...
	floatType          costDif;
	floatType          shiftFlow;
	vector<CLASSPATHSET*> mc_pathset;//各个用户的路径集和需求量
	bool			   skip;//若OD不连通，在计算时需要跳过
...
struct CLASSINFO
{
	...
	void MarkBanlink(){
		for(int a=0;a<BanLinkSet.size();a++)
		{
			BanLinkSet[a]->NoEntry=true;
		}
	}
	...
	int					id;
	floatType			vot;
	vector<TNM_SLINK*>	BanLinkSet;//某一类用户禁止通行的路段；
};
...
public:
	...
	bool				 centroids_blocked; //搜索路径时是否封闭形心；
	vector<TNM_SNODE*>	 centroidSet;
	bool				 IsMultiClass;
	vector<CLASSINFO*>	 multiclassInfo;  //各个用户信息，用于多用户分配
	int					 numOfClass;
	floatType			 period;//Ackelik费用函数的参数，分配的时段长度
...
/*==============================================================================================
  ...
  =============================================================================================*/
	...
	int					 BuildSZFormatPS();//reading SZ format of path search;
	int					 BuildSZFormatPS_T();//reading SZ format for searching transit walking path
	int					 BuildSZFormatAS();//reading SZ format of assignment;
	int					 BuildSZFormatASII();//reading SZ format of assignment;OD文件按O排序，node文件按编号排序
	int					 BuildSZFormatMCAS(const TNM_LINKTYPE tt = BPRLK);//reading SZ format of multi-class assignment;
	...
	TNM_SORIGIN*		 CatchOrgnPtr(TNM_SNODE* onode);
	...
	void				 SetPeriodOfAlkLink(floatType pe);
protected:
...
public:
	...
	queue<TNM_SPATH *>   DIJK_KSP(int orgID, int destID, int K, floatType gap = -1, bool rate = true, bool loop = true);	//loop表示是否允许有环，gap表示可以限制其他路径与最短路的费用差在多少以内，rate表示是否需要保证gap不小于1.0
	queue<TNM_SPATH *>   BFM_KSP(int orgID, int destID, int K, bool loop = true);
	...
	virtual void		 UpdateLinkCost(floatType vot, bool toll = false); //consider first-best toll or not. 
	...
	void                 InitialSubNet4(int sptreeLen = 0, bool createpath = false);//删除不连通的OD
	void                 InitialMCSubNet(int classOrder);

protected:
	int                  BFM_KSP_Search(TNM_SNODE *rootOrg, int K);
	int                  DIJK_KSP_Search(TNM_SNODE *rootOrg, int K, floatType maxCost = -1);
	queue<TNM_SPATH*>    BFM_KSP_GetPath(TNM_SNODE *origin, TNM_SNODE *dest, bool loop = true);
	void                 ReleaseKSPElemMap(); //release memory
