...
floatType TNM_BPRLK::GetDerCost_()
{
	...
	else return (fabs(beta - 0) < 1e-6? 0: fft* alpha * beta * pow(volume/capacity, beta-1)/capacity);
...
{
	if(volume <= 0.0) return 0.0;
	else              return fft*volume *(1.0 + alpha * pow(volume/capacity, beta)/(beta+1));
...
{
	if(volume<=0.0) return 0.0;
	else            return (fabs(beta - 1) < 1e-6? 0.0: fft * alpha * beta * (beta - 1) * pow(volume/capacity,beta - 2) /(capacity * capacity));
}
...
//initilaize origin link and its flows based on shortest path tree.
void TNM_SORIGIN::InitialSubNet(TNM_SNET* net)
{
	ORGLINK* oLink;
	ORGNODE* oNode;
	TNM_SNODE* node;
	tplNodeVector.reserve(net->numOfNode);
	obLinkVector.reserve(net->numOfLink * 0.5);
	for (int i = 0; i < net->numOfNode; i++)
	{
		node = net->nodeVector[i];
		if (node->pathElem->via != NULL || node == origin) //if a node is not accessible from the current origin, no orgNode will be created.
		{
			oNode = new ORGNODE;
			oNode->nodePtr = node;
			node->m_orgNode = oNode;
			oNode->mutliEnd = false;
			oNode->tpLevel = 0;
			oNode->lcnPtr = NULL;
			tplNodeVector.push_back(oNode);
		}
		if (node->pathElem->via != NULL)
		{
			oLink = new ORGLINK;
			assert(oLink != NULL);
			oLink->linkPtr = node->pathElem->via;
			//oLink->oFlow      = 0;
			//oLink->markStatus = 0;
			oLink->approach = 1.0;
			oLink->linkPtr->oLinkPtr = oLink;
			obLinkVector.push_back(oLink);
		} //end if
	} //end for
	vector<ORGNODE*>(tplNodeVector).swap(tplNodeVector); //swap it to save memory.
//	cout<<"Build topological order"<<endl;
	//TopologOrder(net); // build topological order for the current
	//AddPathFromTree(net); //prsumbably shortest path tree has been obtained.
	
	OrgAllOrNothing();
	if (!TopologOrder(net)) return; //update topological order here. 
//	UpdateOrderSubNet(); 
	//m_trimmed = true; ///this makes sure the network get updated
	//TopologOrder(net); //do the topological order once.
	//
	RemarkOBLinksOnNet();
	//	cout<<"Finish Remark"<<endl;
}


void TNM_SORIGIN::MarkOBLinksOnNet()
{
	ORGLINK* oLink;
	ORGNODE* oNode;
	for (std::vector<ORGLINK*>::iterator pv = obLinkVector.begin();
	...
	for (std::vector<ORGNODE*>::iterator pl = tplNodeVector.begin();
		pl != tplNodeVector.end(); pl++)
	{
		oNode = *pl;
		oNode->nodePtr->m_orgNode = oNode;
	}
}

void TNM_SORIGIN::UpdateCFSPTreeOnly()
{
	double gap = 0.0, tgap;
	vector<ORGNODE*>::iterator pv;
	vector<TNM_SLINK*>::iterator pl;
	TNM_SDEST* dest;
	TNM_SNODE* node;
	for (pv = tplNodeVector.begin(); pv != tplNodeVector.end(); pv++)
	{
		node = (*pv)->nodePtr;
		node->pathElem->cost = POS_INF_FLOAT;
		node->pathElem->via = NULL;
	}
	tplNodeVector[0]->nodePtr->pathElem->cost = 0.0;
	double t;
	for (pv = tplNodeVector.begin(); pv != tplNodeVector.end(); pv++)
	{
		node = (*pv)->nodePtr;
		for (pl = node->forwStar.begin(); pl != node->forwStar.end(); pl++)
		{

			if ((*pl)->oLinkPtr != NULL) //only consider links on the network.
			{
				//(*pv)->f
				if ((t = node->pathElem->cost + (*pl)->cost) < (*pl)->head->pathElem->cost)
				{
					(*pl)->head->pathElem->cost = t;
					(*pl)->head->pathElem->via = (*pl);
				}
			}
		}
	}
...
TNM_SORIGIN* TNM_SNET::CatchOrgPtr(int id)
{
	vector<TNM_SORIGIN*>::iterator pv;
	pv = find_if(originVector.begin(), originVector.end(), predP(&TNM_SORIGIN::id_, id));
	if (pv == originVector.end()) return NULL;
	else                         return *pv;
}

void TNM_SORIGIN::RemarkOBLinksOnNet()
{
	ORGLINK* oLink;
		...
		pv != obLinkVector.end(); pv++)
	{
		...
		else if (oLink->linkPtr==NULL)
		{
		cout<<"olink->linkptr is null"<<endl;
		}*/
		oLink->linkPtr->oLinkPtr = NULL;
	}
}

/* =======================================================================================
	Standard topological ordering algorithm
	If the network is acyclic, it will produce a topological order, and save it
	in tplNodeVector. Otherwise, it will return false.
   ======================================================================================*/
bool TNM_SORIGIN::TopologOrder(TNM_SNET* net)
{
	/*	if(!NeedOrderSubNet())
		{
			//cout<<"\tno need to reorder subnet"<<endl;
			//getchar();
			return true;
		}*/
		//cout<<"begin topological order"<<endl;
	TNM_SNODE* node, * node0, * node1;
	TNM_SLINK* link;
	int zeroIn = 1;
	...
	{
		node = net->nodeVector[i];
		node->tmpNumOfIn = node->NumOfOrgInLink();
		if (node->tmpNumOfIn == 0 && node != origin)  //for such node, no orgnode associated except origin.
		{
			zeroIn++;
			//node->m_orgNode->tpLevel = net->numOfNode + 1;
		}
		else
		{
			node->m_orgNode->lcnPtr = NULL;
			node->m_orgNode->tpLevel = 0;
			if (node->tmpNumOfIn > 1) node->m_orgNode->mutliEnd = true;
			else node->m_orgNode->mutliEnd = false;
		}

	}
	//tplNodeVector.clear();
//	cout<<"clear all pointers"<<endl;
	tplNodeVector.clear();
	queue<TNM_SNODE*> nq;
	vector<TNM_SLINK*>::iterator pv;
	vector<TNM_SNODE*>::iterator pn;
	nq.push(origin);
	tplNodeVector.push_back(origin->m_orgNode);
	origin->scanStatus = 1;
	int tL, count = 0;
	...
	while (!nq.empty() && count <= net->numOfNode)
	{
		...
		node = nq.front();
		nq.pop();
		for (pv = node->forwStar.begin(); pv != node->forwStar.end(); pv++)
		{
			link = *pv;
			if (link && link->oLinkPtr)
			{
				...
				{
					nq.push(link->head);
					link->head->m_orgNode->tpLevel = node->m_orgNode->tpLevel + 1;
					tplNodeVector.push_back(link->head->m_orgNode);
				}
				/*if((tL = node->m_orgNode->tpLevel + 1) > link->head->m_orgNode->tpLevel)
					link->head->m_orgNode->tpLevel = tL;*/
			}
		}
	}
	//cout<<"finish sorting"<<endl;
	if (count < net->numOfNode - zeroIn + 1)
	{
		cout << "\tFail to create topological order: network may contain cycles." << endl;
		return false;
	}
	//sort(tplNodeVector.begin(), tplNodeVector.end(), TPLOrderCompare); //sort it.

	/*queue<TNM_SNODE *> zeroInNode; //node list who include on incoming links;
	vector<TNM_SLINK*>::iterator pv;
	tplNodeVector.clear(); //make the topological order node empty
	int lev = 0;
	int nZeroIn =0;
	for (int i = 0;i<net->numOfNode;i++)
	{
		node = net->nodeVector[i];
		node->InitNumOfIn(); //only those links whose orgLink !=NULL will be count.
		//if(node->tmpNumOfIn ==0) numNoIn ++;
		//node->buffer[1] = POS_INF_FLOAT;
		node->m_orgNode->mutliEnd = false;
		node->m_orgNode->lcnPtr   = NULL;
		node->m_orgNode->tpLevel  = 0;
		if(node->tmpNumOfIn == 0)
		{
			nZeroIn ++;
		}
		if(node->tmpNumOfIn > 1)
		{
			node->m_orgNode->mutliEnd = true;
		}
	}
	if(origin->tmpNumOfIn > 0)
	{
		cout<<"origin "<<origin->id<<" has incoming link! This is not permitted."<<endl;
		for(PTRTRACE pv = origin->backStar.begin(); pv!= origin->backStar.end(); pv++)
		{
			link = *pv;
			if((*pv) && (*pv)->oLinkPtr)
			{
				cout<<"incoming link "<<link->id<<" head cost = "<<link->head->pathElem->cost
					<<" tail cost = "<<link->tail->pathElem->cost<<" link cost  "<<link->cost<<endl;
			}
		}
		getchar();
	}
	origin->m_orgNode->tpLevel = lev;
	zeroInNode.push(origin);
	int nScannedLink = 0;
	while (!zeroInNode.empty() && nScannedLink <= net->numOfLink)
	{
		node = zeroInNode.front(); // take the first node from zeroInNode
		zeroInNode.pop(); //delete it from vector
		tplNodeVector.push_back(node->m_orgNode); // insert it to the vector preserving topological order
		for (pv = node->forwStar.begin(); pv!= node->forwStar.end(); pv++) //scan forward star
		{
			link = *pv;
			nScannedLink ++;
			if(link!=NULL)
			{
				if(link->oLinkPtr != NULL) //only for those link that is on the network.
				{
					link->head->tmpNumOfIn --;
					if (link->head->tmpNumOfIn ==0)
					{
						zeroInNode.push(link->head);
						link->head->m_orgNode->tpLevel = node->m_orgNode->tpLevel + 1;
					}
				}
			}
	...
	if (tplNodeVector.size()<net->numOfNode - nZeroIn + 1)
	{
		cout<<"A cycle might exist for origin "<<origin->id_()
			<<" Total number of node without incoming link ="<<nZeroIn
			<<" Node in topological order = "<<tplNodeVector.size()
			<<" total node size = "<<net->numOfNode<<endl;
		cout<<" nodes in topological order: "<<endl;
		getchar();
		return false;
	}
		*/
	...
	ORGNODE* onode;
	vector<ORGNODE*>::iterator po;


	//now find out the last common node for each node.
	for (po = tplNodeVector.begin(); po != tplNodeVector.end(); po++)
	{
		onode = *po;
		node = onode->nodePtr;
		//node->pthHeap = NULL;
		for (PTRTRACE pv1 = node->backStar.begin(); pv1 != node->backStar.end(); pv1++)
		{
			link = *pv1;
			if (link)
			{
				if (link->oLinkPtr)
				{
					//cout<<"examine node "<<node->id<<endl;
					if (!onode->mutliEnd) //single termination node.
					{
						////cout<<"single termination- set its LCN to "<<link->tail->id<<endl;
						onode->lcnPtr = link->tail;//we actually store a link pointer but force it to a node.
						//node->kspPathElem  = (KSPMAP*) link; //for temporary use.
					}
					else
					{
						if (onode->lcnPtr == NULL)   //for the first link, just plug in its tail node

						{
							//cout<<"multiple termination first visit - set its first LCN to "<<link->tail->id<<endl;
							onode->lcnPtr = link->tail;
						}
						else
						{
							node0 = onode->lcnPtr;
							...
							while (node0 != node1)
							{
								//cout<<"repleat visit: node0 = "<<node0->id<<" at lev  "<<node0->tmpNumOfIn
								//	<<" node 1= "<<node1->id<<" at lev "<<node1->tmpNumOfIn<<endl;
								if (node0->m_orgNode->tpLevel == node1->m_orgNode->tpLevel)
								{
									node0 = node0->m_orgNode->lcnPtr;
									node1 = node1->m_orgNode->lcnPtr;

								}
								else
								{
									while (node0->m_orgNode->tpLevel > node1->m_orgNode->tpLevel)
									{
										node0 = node0->m_orgNode->lcnPtr;
									}
									while (node0->m_orgNode->tpLevel < node1->m_orgNode->tpLevel)
							...
							//cout<<"multiple termination repeated visit - set its first LCN to "<<node0->id<<endl;
							node->m_orgNode->lcnPtr = node0;
						}
					}
					//getchar();
				}
	...
	for (po = tplNodeVector.begin(); po != tplNodeVector.end(); po++)
	{
		...
		for (PTRTRACE pv1 = node->backStar.begin(); pv1 != node->backStar.end(); pv1++)
		{
			...
			{
				if (!onode->mutliEnd)
					onode->lcnPtr = (TNM_SNODE*)link;
			}
		}
	}
	//UpdateOrderSubNet(false);
	return true;
}

void TNM_SORIGIN::OrgAllOrNothing()
{
	TNM_SDEST* dest;
	TNM_SNODE* curNode;
	TNM_SLINK* via;
	...
	for (int i = 0; i < numOfDest; i++)
	{
		...
		dmd = dest->assDemand;
		curNode = dest->dest;
		while (curNode != this->origin)
		{
			via = curNode->pathElem->via;
			via->oLinkPtr->oFlow += dmd;
			via->volume += dmd;
			curNode = via->tail;
		}
	}
...
void TNM_SNET::InitialSubNet(int kx, bool createPath)
{
	TNM_SORIGIN* origin;
	/*	for (int i = 0;i<numOfLink;i++)
		{
			linkVector[i]->CatchRevLink();
		}*/
	for (int i = 0; i < numOfOrigin; i++)
	{
